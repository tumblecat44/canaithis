...
smoke() {
  ...
  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?category=automation&sort=popular" || echo "000")
  log "smoke /xx?category=automation&sort=popular (invalid locale home + automation category + popular sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?category=automation&sort=views" || echo "000")
  log "smoke /xx?category=automation&sort=views (invalid locale home + automation category + views sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?page=2&category=automation" || echo "000")
  log "smoke /xx?page=2&category=automation (invalid locale home + pagination + automation category) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?page=2&category=automation&sort=latest" || echo "000")
  log "smoke /xx?page=2&category=automation&sort=latest (invalid locale home + pagination + automation category + latest sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?page=2&category=automation&sort=popular" || echo "000")
  log "smoke /xx?page=2&category=automation&sort=popular (invalid locale home + pagination + automation category + popular sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?page=2&category=automation&sort=views" || echo "000")
  log "smoke /xx?page=2&category=automation&sort=views (invalid locale home + pagination + automation category + views sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?page=2&q=test&category=automation" || echo "000")
  log "smoke /xx?page=2&q=test&category=automation (invalid locale home + pagination + search + automation category) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?page=2&q=test&category=automation&sort=latest" || echo "000")
  log "smoke /xx?page=2&q=test&category=automation&sort=latest (invalid locale home + pagination + search + automation category + latest sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?page=2&q=test&category=automation&sort=popular" || echo "000")
  log "smoke /xx?page=2&q=test&category=automation&sort=popular (invalid locale home + pagination + search + automation category + popular sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?page=2&q=test&category=automation&sort=views" || echo "000")
  log "smoke /xx?page=2&q=test&category=automation&sort=views (invalid locale home + pagination + search + automation category + views sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?q=test&category=automation" || echo "000")
  log "smoke /xx?q=test&category=automation (invalid locale home + search + automation category) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?q=test&category=automation&sort=views" || echo "000")
  log "smoke /xx?q=test&category=automation&sort=views (invalid locale home + search + automation category + views sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?q=test&category=automation&sort=latest" || echo "000")
  log "smoke /xx?q=test&category=automation&sort=latest (invalid locale home + search + automation category + latest sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx?q=test&category=automation&sort=popular" || echo "000")
  log "smoke /xx?q=test&category=automation&sort=popular (invalid locale home + search + automation category + popular sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1
  ...
  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?category=automation" || echo "000")
  log "smoke /xx/challenges?category=automation (invalid locale nested + automation category) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?category=automation&sort=latest" || echo "000")
  log "smoke /xx/challenges?category=automation&sort=latest (invalid locale nested + automation category + latest sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?category=automation&sort=popular" || echo "000")
  log "smoke /xx/challenges?category=automation&sort=popular (invalid locale nested + automation category + popular sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?category=automation&sort=views" || echo "000")
  log "smoke /xx/challenges?category=automation&sort=views (invalid locale nested + automation category + views sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?q=test&category=automation" || echo "000")
  log "smoke /xx/challenges?q=test&category=automation (invalid locale nested + search + automation category) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?q=test&category=automation&sort=latest" || echo "000")
  log "smoke /xx/challenges?q=test&category=automation&sort=latest (invalid locale nested + search + automation category + latest sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?q=test&category=automation&sort=popular" || echo "000")
  log "smoke /xx/challenges?q=test&category=automation&sort=popular (invalid locale nested + search + automation category + popular sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?q=test&category=automation&sort=views" || echo "000")
  log "smoke /xx/challenges?q=test&category=automation&sort=views (invalid locale nested + search + automation category + views sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1
  ...
  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?page=2&category=automation" || echo "000")
  log "smoke /xx/challenges?page=2&category=automation (invalid locale nested + pagination + automation category) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?page=2&category=automation&sort=latest" || echo "000")
  log "smoke /xx/challenges?page=2&category=automation&sort=latest (invalid locale nested + pagination + automation category + latest sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?page=2&category=automation&sort=popular" || echo "000")
  log "smoke /xx/challenges?page=2&category=automation&sort=popular (invalid locale nested + pagination + automation category + popular sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?page=2&category=automation&sort=views" || echo "000")
  log "smoke /xx/challenges?page=2&category=automation&sort=views (invalid locale nested + pagination + automation category + views sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?page=2&q=test&category=automation" || echo "000")
  log "smoke /xx/challenges?page=2&q=test&category=automation (invalid locale nested + pagination + search + automation category) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?page=2&q=test&category=automation&sort=latest" || echo "000")
  log "smoke /xx/challenges?page=2&q=test&category=automation&sort=latest (invalid locale nested + pagination + search + automation category + latest sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?page=2&q=test&category=automation&sort=popular" || echo "000")
  log "smoke /xx/challenges?page=2&q=test&category=automation&sort=popular (invalid locale nested + pagination + search + automation category + popular sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1

  code=$(curl -sL -o /dev/null -w "%{http_code}" "${PROD_URL}/xx/challenges?page=2&q=test&category=automation&sort=views" || echo "000")
  log "smoke /xx/challenges?page=2&q=test&category=automation&sort=views (invalid locale nested + pagination + search + automation category + views sort) → ${code}"
  [[ "$code" == "404" ]] || ok=1
